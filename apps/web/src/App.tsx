...
function App() {
  ...
  return <main className="app-shell">
    ...
    {page === "model-settings" ? <section className="panel model-settings"><div className="section-heading"><p className="eyebrow">P14-D Model Settings</p><h2>模型設定 / Model Settings</h2></div><p className="lock-note">{modelStatus}</p>
      ...
      <p className="lock-note">若留空，將保留已儲存的 API Key。若要更換，請輸入新的 API Key。若要刪除，請按「清除 API Key」。 Leave blank to keep the saved API key. Enter a new key to replace it. Use “Clear API Key” to remove it.</p>
      ...
      <div className="action-grid"><button type="button" onClick={() => run("儲存設定", () => api("/api/settings/model", { method: "POST", body: JSON.stringify(modelPayload()) }))}>儲存設定</button><button type="button" onClick={() => run("測試連線", () => api("/api/model/test", { method: "POST", body: JSON.stringify(modelPayload()) }))}>測試連線</button><button type="button" onClick={() => run("清除 API Key", () => api("/api/settings/model", { method: "POST", body: JSON.stringify(clearApiKeyPayload()) }))} disabled={form.provider !== "openai_compatible"}>清除 API Key</button><button type="button" onClick={switchSandbox}>切回沙盒模式</button></div><p className="lock-note">{message || "沙盒模式已啟用時，不呼叫外部模型，不需要 API key。API key 只以遮罩狀態顯示。"}</p></section> : <>
