...
const workbenchCommands = ["生成任務確認單", "建立任務確認單", "建立確認單", "生成確認單", "送到工作台", "轉成工作台任務", "建立 SCBKR 任務", "建立 SCBKR 確認單"];
...
export default function App() {
  ...
  const sendChat = async () => { const user = chatInput.trim(); if (!user) return; const next = [...messages, { role: "user", content: user }]; setMessages(next); setChatInput(""); if (isWorkbenchCommand(user)) { const taskInput = cleanWorkbenchCommand(user) || user; setTaskText(taskInput); const created = await run("建立確認單", () => api<TaskSummary>("/api/tasks/create", { method: "POST", body: JSON.stringify({ raw_input: taskInput, task_type: taskType, create_scbkr_draft: true, prefill: { user_original: user, suggested_instruction: taskInput, suggested_type: "工作台任務", suggested_reason: "使用者在聊天中明確要求建立任務確認單。", suggested_write_direction: "暫不入庫" } }) })); if (created?.task_id) { setTask(created); setPage("workbench"); setDrawer(false); setMessages([...next, { role: "assistant", content: "已建立工作台任務確認單，請到右側 Workbench 檢查 S/C/B/K/R 責任鏈草案。確認前不會生成，驗收前不會入庫。" }]); } return; } const r = await run("一般聊天", () => api<Record<string, any>>("/api/chat/general", { method: "POST", body: JSON.stringify({ message: user }) })); if (r) { setMessages([...next, { role: "assistant", content: r.reply }]); setSuggestion(r.suggestion); } };
  ...
  const createTask = async () => { if (!taskText.trim()) { setMessage("請先輸入任務內容。"); return; } return run("建立確認單", () => api<TaskSummary>("/api/tasks/create", { method: "POST", body: JSON.stringify({ raw_input: taskText.trim(), task_type: taskType, create_scbkr_draft: true, prefill }) })).then((r: any) => r?.task_id && setPage("workbench")); };
  ...
  const dataItems = (section: string): Record<string, any>[] => { const source: any = dataCenter || {}; const keys: Record<string, string[]> = { "任務紀錄": ["tasks", "task_records"], "確認單": ["confirmations", "scbkr_confirmations"], "生成結果": ["generations", "generation_results"], "驗收紀錄": ["reviews", "review_records"], "入庫資料": ["storage_items", "stored_items"], "向量庫": ["vector", "vector_store"], "語料庫": ["corpus"], "程式邏輯庫": ["logic"], "記憶庫": ["memory", "memory_rules"], "回放帳本": ["ledger", "replay_ledger"] }; for (const key of keys[section] || []) { const value = source[key]; if (Array.isArray(value)) return value; if (value && Array.isArray(value.items)) return value.items; } return []; };
