...
export default function App() {
  ...
  const [modelForm, setModelForm] = useState({ provider: "lm_studio", mode: "local", base_url: "http://127.0.0.1:1234/v1", api_key: "local", model_name: "", temperature: 0.2, max_tokens: 4096, context_length: 8192, timeout: 120 });

  const refresh = async () => { try { await api("/health"); setHealth("online"); const m = await api<ModelSettings>("/api/settings/model"); setModel(m); setModelForm({ ...modelForm, provider: m.provider || modelForm.provider, mode: m.mode || modelForm.mode, base_url: m.base_url || modelForm.base_url, model_name: m.model_name || modelForm.model_name }); } catch (e) { setHealth("offline"); setMessage(String(e)); } };
