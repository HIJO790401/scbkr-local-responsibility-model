...
def _task_type(user_input: str) -> str:
    ...
    if any(token in text for token in ("美容", "臉部", "脸部", "保養", "保养", "文案", "貼文", "贴文", "copy")):
    ...
    *,
    ...
    for hit in hits:
        ...
        if _formal(hit) and source_store == "logic":
            ...
            non_citable_data.append({**item, "reason": "檢索庫只負責召回候選，不可直接當正式依據。"})
        else:
            non_citable_data.append({**item, "reason": "未完成簽名、驗收、啟用或相關性不足。"})
    ...
    if task_type == "beauty_salon_marketing_copy":
        forbidden_actions.extend(["不得誇大療效。", "不得編造價格、優惠、療程時間或客戶見證。"])
        output_limits.extend(["美容文案不得暗示醫療效果或保證結果。", "沒有價格資料時不得寫價格。"])
        stop_conditions.extend(["需要價格、療效、優惠期限或客戶案例時必須追問或標待確認。"])
    ...
    package = {
        ...
        "missing_information": [] if matched_rules else ["沒有命中已簽名規則時，只能一般聊天或草稿。"],
