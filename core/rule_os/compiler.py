...
def compile_executable_rule(task: dict[str, Any], storage_items: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "signature_ref": scbkr.get("confirmed_snapshot_hash"),
        "rule_trigger_contract": scbkr.get("rule_trigger_contract"),
        "review_passed": task.get("review_passed") is True,
        "version": int(task.get("revision_number") or 1),
        "supersedes": task.get("supersedes_rule_id"),
