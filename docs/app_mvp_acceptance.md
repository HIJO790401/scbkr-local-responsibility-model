...
### P13-C merged retrieval acceptance

- ChromaDB non-empty results must not hide SQLite fallback-only exact matches.
- Fallback-only cases stored in SQLite must remain searchable even when ChromaDB returns candidates.
- Merged retrieval candidates must include deterministic `score` and `route` values.
- Retrieval remains advisory and must not auto-confirm, auto-generate, or auto-commit storage.

### P14-A Sandbox Mode acceptance

- Sandbox mode does not call external APIs, LM Studio, or Ollama.
- Sandbox mode does not require an API key or model download.
- Sandbox mode still requires confirmed SCBKR sealed snapshots before generation.
- Sandbox mode still requires `model_generate` permission.
- Sandbox output is clearly marked with `sandbox=true` and `provider=sandbox_mock_model`.
- Sandbox output cannot bypass review or storage gates and cannot auto-write memory.

### P13-C SQLite fallback completeness acceptance

- SQLite fallback retrieval must score all relevant durable rows before final `top_k` truncation.
- Older SQLite-only exact matches must remain searchable even when more than 200 newer retrieval cases exist.
- Non-empty ChromaDB results must not hide older SQLite fallback exact matches.

### P14-B Desktop Launch Skeleton acceptance

- Desktop skeleton exists under `apps/desktop`.
- Desktop status endpoint exists and returns P14-B status.
- Desktop status does not perform generation, storage writes, retrieval confirmation, or external calls.
- Sandbox remains available.
- Local LM Studio endpoint remains optional and user-provided.
- No installer is built in P14-B.

### P14-C Windows Desktop Preview acceptance

- Windows preview artifact path exists through scripts and GitHub Actions.
- Preview users should not need Python or Node when using the produced artifact.
- Desktop app starts or supervises the FastAPI sidecar on `127.0.0.1:8787`.
- Desktop app loads the packaged Web UI build output.
- Sandbox workflow completes without a model or API key.
- LM Studio local test path is documented and optional.
- No external call happens unless the user explicitly runs model test or generate with local/external settings.
- Storage, memory, retrieval, P10, and P12 gates remain intact.
