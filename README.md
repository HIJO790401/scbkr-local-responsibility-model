...
P12 目前保留「一鍵確認責任鏈」操作，但底層不再只是設定單一 `confirmed` boolean。一鍵確認會批次確認 S / C / B / K / R 五個維度，並為每一維封存 `confirmed_at`、`confirmed_by`、`snapshot_hash` 與 `confirmed_snapshot`，讓當時確認的五維責任鏈版本可回放、可審計。generate 前也必須通過五維全部 confirmed 的安全閘門。
