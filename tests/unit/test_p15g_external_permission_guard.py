...
def test_remote_chat_calls_model_when_external_api_allowed(monkeypatch):
    ...
    assert "Four-store state" in response.json()["reply"]
    assert "no signed citation" in response.json()["reply"]
    assert response.json()["rule_state"]["awareness_state"] in {"EMPTY", "DRAFTING"}
