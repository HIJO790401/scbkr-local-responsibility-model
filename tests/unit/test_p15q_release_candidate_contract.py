...
def test_release_build_is_free_model_assisted_product_not_legacy_preview():
    assert 'public_edition = "FREE"' in RELEASE_BUILD
    assert 'model_assisted_rulebook_required = $true' in RELEASE_BUILD
    assert 'README_RELEASE.md' in RELEASE_BUILD
...
def test_release_candidate_metadata_and_readme_contracts():
    ...
    assert '"version": "2.3.0"' in desktop_package
