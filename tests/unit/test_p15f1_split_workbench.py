...
def test_fake_model_valid_draft_is_written_to_task_and_workbench(monkeypatch):
    ...
    task = response.json()
    assert task["scbkr"]["S"]["task_name"] == "FAKE_MODEL_DRAFT_紫蘇梅冰沙開幕宣傳"
    assert task["scbkr"]["S"]["output_format"] == "FAKE_MODEL_OUTPUT_標題_短文案_CTA"
    assert task["scbkr"]["fallback_used"] is False
    readback = main.get_task(task["task_id"])
    assert readback["scbkr"]["S"]["task_name"] == "FAKE_MODEL_DRAFT_紫蘇梅冰沙開幕宣傳"
...
def test_remote_external_disabled_fallback_and_loopback_not_blocked(monkeypatch):
    ...
    assert r.json()["scbkr"]["fallback_used"] is True
    assert r.json()["scbkr"]["fallback_reason"] == "external_api_permission_disabled"
